...
static void drawTextbox(Display *dpy, Window win, int screen, int len, unsigned long bgcolor)
{
	int x, y, width, height, s_width, s_height, i;
    char message[50], placeholder[30];
	XGCValues gr_values;
	XFontStruct *fontinfo;
	XColor fgcolor, dummy;
	XineramaScreenInfo *xsi;
	GC gc, gc2;
	fontinfo = XLoadQueryFont(dpy, "-adobe-courier-medium-r-normal--18-180-75-75-m-110-iso8859-14");
	...
	XAllocNamedColor(dpy, DefaultColormap(dpy, screen),
		 "#222222", &fgcolor, &dummy);

	gr_values.font = fontinfo->fid;
	gr_values.foreground = fgcolor.pixel;
	gc=XCreateGC(dpy,win,GCFont+GCForeground, &gr_values);
    gr_values.foreground = bgcolor;
    gc2 = XCreateGC(dpy, win, GCForeground, &gr_values);
    ...
    len = len > 30 ? 30 : len;
    for (i = 0; i < len; i++) {
        message[i] = passwd_char;
    }
    message[i] = '\n';
    for (i = 0; i < sizeof(placeholder); i++) {
        placeholder[i] = passwd_char;
    }
	...
	height = fontinfo->ascent + fontinfo->descent + 10;
	width  = XTextWidth(fontinfo, placeholder, sizeof(placeholder)) + 20;
    x = (s_width - width) / 2;
    y = s_height * 1/2;

    XFillRectangle(dpy, win, gc2, x - 10, y - fontinfo->ascent - 5, width, height);
            ...
            if (running && oldc != color) {
				for (screen = 0; screen < nscreens; screen++) {
                    ...
                    if (locks[screen]->bgmap)
                        ...
                        XSetWindowBackgroundPixmap(dpy, locks[screen]->win, locks[screen]->colors[color]);
                }
                oldc = color;
            }
			if (running) {
				for (screen = 0; screen < nscreens; screen++) {
                    ...
                    drawTextbox(dpy, locks[screen]->win, screen, len, locks[screen]->colors[color]);
				}
			}
...
main(int argc, char **argv) {
	...
	for (nlocks = 0, s = 0; s < nscreens; s++) {
		if ((locks[s] = lockscreen(dpy, &rr, s)) != NULL) {
            ...
            drawTextbox(dpy, locks[s]->win, s, 0, locks[s]->colors[INIT]);
			nlocks++;
		} else {
			break;
		}
